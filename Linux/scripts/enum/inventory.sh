...
if command -v yum >/dev/null ; then
  ...
  OPENBSD
elif uname -s 2>/dev/null | grep -qi '^sunos'; then
  # Solaris detection (must come before FreeBSD pkg check since Solaris 11+ also has pkg)
  SOLARIS
elif uname -s 2>/dev/null | grep -qiE '^(NodeOS|WeirdOS)'; then
  NODEOS
...
if [ -z "$RAM" ]; then
    ...
    elif [ $IS_SOLARIS = true ]; then
        # Solaris: prtconf reports memory in MB
        RAM=$( DPRINT prtconf 2>/dev/null | grep '^Memory size:' | awk '{print $3, $4}' )
    else
...
elif [ $IS_SOLARIS = true ] && command -v netstat >/dev/null; then
    # Solaris: netstat -an lists both TCP and UDP
    DPRINT netstat -an -P tcp | grep 'LISTEN' | DPRINT column -t
...
if [ $IS_ALPINE = true ]; then
    ...
    # Solaris SMF: list online services
    if command -v svcs >/dev/null 2>&1; then
        SERVICES=$( DPRINT svcs -a 2>/dev/null | grep '^online' | awk '{print $NF}' )
    fi
elif [ $IS_NODEOS = true ]; then
    # NodeOS: npm-based service list
    if command -v npm >/dev/null 2>&1; then
        SERVICES=$( DPRINT npm list --global --depth=0 2>/dev/null | grep -v '^/' | awk '{print $2}' | cut -d'@' -f1 )
    fi
else
...
${ECHO} "\n${GREEN}#############ROCKET.CHAT ENUMERATION############${NC}\n"

RC_FOUND=false

if checkService "$SERVICES" 'rocketchat' 'Rocket.Chat' | grep -qi "is on this machine" || \
   DPRINT pgrep -f 'rocketchat\|Rocket.Chat\|main.js.*rocketchat' >/dev/null 2>&1 || \
   DPRINT ps aux 2>/dev/null | grep -qi 'rocketchat\|Rocket\.Chat'; then
    RC_FOUND=true
    checkService "$SERVICES" 'rocketchat' 'Rocket.Chat' 2>/dev/null || ${ECHO} "${BLUE}[+] Rocket.Chat process detected${NC}"
fi

if command -v snap >/dev/null 2>&1 && DPRINT snap list 2>/dev/null | grep -qi 'rocketchat'; then
    RC_FOUND=true
    ${ECHO} "${BLUE}[+] Rocket.Chat snap package detected${NC}"
fi

if [ "$RC_FOUND" = true ]; then
    ${ECHO} "${YELLOW}[*] Checking LDAP Configuration...${NC}\n"

    # 1. Env vars in current environment
    RC_LDAP_ENV=$( env 2>/dev/null | grep -iE '^LDAP_' )
    if [ -n "$RC_LDAP_ENV" ]; then
        ${ECHO} "${BLUE}[+] LDAP env vars in current environment:${NC}"
        ${ECHO} "${ORAG}$RC_LDAP_ENV${NC}\n"
    fi

    # 2. .env and docker-compose files
    RC_ENV_FILES=$( DPRINT find /opt /var /home /root /snap -maxdepth 6 \
        \( -name '.env' -o -name 'docker-compose.yml' -o -name 'docker-compose.yaml' \) \
        2>/dev/null | head -n 20 )
    for rcfile in $RC_ENV_FILES; do
        RC_LDAP_LINES=$( grep -iE 'LDAP' "$rcfile" 2>/dev/null )
        if [ -n "$RC_LDAP_LINES" ]; then
            ${ECHO} "${BLUE}[+] LDAP settings in $rcfile:${NC}"
            ${ECHO} "${ORAG}$RC_LDAP_LINES${NC}\n"
        fi
    done

    # 3. Systemd unit files
    RC_UNIT_FILES=$( DPRINT find /etc/systemd /lib/systemd -name '*rocketchat*' -o -name '*rocket*' 2>/dev/null )
    for ufile in $RC_UNIT_FILES; do
        RC_UNIT_LDAP=$( grep -iE 'LDAP' "$ufile" 2>/dev/null )
        if [ -n "$RC_UNIT_LDAP" ]; then
            ${ECHO} "${BLUE}[+] LDAP in systemd unit $ufile:${NC}"
            ${ECHO} "${ORAG}$RC_UNIT_LDAP${NC}\n"
        fi
    done

    # 4. MongoDB query (Rocket.Chat stores LDAP config in DB)
    MONGO_CMD=""
    if command -v mongosh >/dev/null 2>&1; then MONGO_CMD="mongosh --quiet"
    elif command -v mongo >/dev/null 2>&1; then MONGO_CMD="mongo --quiet"
    fi

    if [ -n "$MONGO_CMD" ]; then
        ${ECHO} "${BLUE}[+] Querying MongoDB for LDAP settings...${NC}"
        for rcdb in rocketchat meteor; do
            RC_LDAP_DB=$( DPRINT $MONGO_CMD "$rcdb" --eval \
                'db.rocketchat_settings.find({_id:/LDAP/},{_id:1,value:1}).forEach(function(d){print(d._id+"="+d.value)})' \
                2>/dev/null | grep -v '^$' )
            if [ -n "$RC_LDAP_DB" ]; then
                ${ECHO} "${YELLOW}LDAP from MongoDB ($rcdb):${NC}"
                LDAP_ENABLE_LINE=$( echo "$RC_LDAP_DB" | grep -i 'LDAP_Enable=' )
                if echo "$LDAP_ENABLE_LINE" | grep -qi '=true\|=1'; then
                    ${ECHO} "${RED}[!] LDAP is ENABLED${NC}"
                else
                    ${ECHO} "${GREEN}[-] LDAP appears DISABLED${NC}"
                fi
                ${ECHO} "${ORAG}$RC_LDAP_DB${NC}\n"
                break
            fi
        done
    else
        ${ECHO} "${YELLOW}[-] mongo/mongosh not available — manually run:${NC}"
        ${ECHO} "${YELLOW}    db.rocketchat_settings.find({_id:/LDAP/})${NC}\n"
    fi
else
    ${ECHO} "${YELLOW}[-] Rocket.Chat not detected on this machine${NC}\n"
fi

${ECHO} "\n${GREEN}#############HASHICORP VAULT ENUMERATION############${NC}\n"

VAULT_FOUND=false

if command -v vault >/dev/null 2>&1; then
    VAULT_FOUND=true
    ${ECHO} "${BLUE}[+] HashiCorp Vault binary detected${NC}"
    ${ECHO} "${YELLOW}Version: $( DPRINT vault version 2>/dev/null )${NC}\n"
fi

if checkService "$SERVICES" 'vault' | grep -qi "is on this machine"; then
    VAULT_FOUND=true
    checkService "$SERVICES" 'vault'
fi

if [ "$VAULT_FOUND" = true ]; then

    # Config files
    ${ECHO} "${BLUE}[+] Vault Config Files:${NC}"
    VAULT_CFG_LOCS="/etc/vault.d/vault.hcl /etc/vault/vault.hcl /opt/vault/config/vault.hcl /etc/vault.d/config.hcl"
    VAULT_CFG_EXTRA=$( DPRINT find /etc /opt /var -name '*.hcl' -path '*vault*' 2>/dev/null | head -n 5 )
    for vcfg in $VAULT_CFG_LOCS $VAULT_CFG_EXTRA; do
        if [ -f "$vcfg" ]; then
            ${ECHO} "${YELLOW}$vcfg:${NC}"
            ${ECHO} "${ORAG}$( grep -vE '^\s*(#|$)' "$vcfg" 2>/dev/null )${NC}\n"
        fi
    done

    # Environment variables (may contain token/addr)
    ${ECHO} "${BLUE}[+] Vault Environment Variables:${NC}"
    VAULT_ENV=$( env 2>/dev/null | grep -iE '^VAULT_' )
    if [ -n "$VAULT_ENV" ]; then
        ${ECHO} "${RED}[!] Vault env vars found (may contain credentials):${NC}"
        ${ECHO} "${ORAG}$VAULT_ENV${NC}\n"
    else
        ${ECHO} "${YELLOW}[-] No VAULT_* environment variables set${NC}\n"
    fi

    # .vault-token files in home dirs
    ${ECHO} "${BLUE}[+] Vault Token Files:${NC}"
    VAULT_TOKENS=""
    for homedir in /root /home/*; do
        if [ -f "$homedir/.vault-token" ]; then
            TOKEN=$( cat "$homedir/.vault-token" 2>/dev/null )
            VAULT_TOKENS="${VAULT_TOKENS}\n  $homedir/.vault-token: $TOKEN"
        fi
    done
    if [ -n "$VAULT_TOKENS" ]; then
        ${ECHO} "${RED}[!] Vault token files:${NC}"
        ${ECHO} "${ORAG}$VAULT_TOKENS${NC}\n"
    else
        ${ECHO} "${YELLOW}[-] No .vault-token files in home dirs${NC}\n"
    fi

    # Vault status (seal/init state)
    ${ECHO} "${BLUE}[+] Vault Status:${NC}"
    VAULT_STATUS=$( DPRINT vault status 2>/dev/null )
    if [ -n "$VAULT_STATUS" ]; then
        if echo "$VAULT_STATUS" | grep -qi 'Sealed.*false'; then
            ${ECHO} "${RED}[!] Vault is UNSEALED${NC}"
        elif echo "$VAULT_STATUS" | grep -qi 'Sealed.*true'; then
            ${ECHO} "${YELLOW}[*] Vault is SEALED${NC}"
        fi
        ${ECHO} "${ORAG}$VAULT_STATUS${NC}\n"
    else
        ${ECHO} "${YELLOW}[-] Could not retrieve vault status (check VAULT_ADDR)${NC}\n"
    fi

    # Auth methods
    ${ECHO} "${BLUE}[+] Vault Auth Methods:${NC}"
    VAULT_AUTH=$( DPRINT vault auth list 2>/dev/null )
    if [ -n "$VAULT_AUTH" ]; then
        ${ECHO} "${ORAG}$VAULT_AUTH${NC}\n"
    else
        ${ECHO} "${YELLOW}[-] Cannot list auth methods (not authenticated or sealed)${NC}\n"
    fi

    # Secrets engines
    ${ECHO} "${BLUE}[+] Vault Secrets Engines:${NC}"
    VAULT_SECRETS=$( DPRINT vault secrets list 2>/dev/null )
    if [ -n "$VAULT_SECRETS" ]; then
        ${ECHO} "${ORAG}$VAULT_SECRETS${NC}\n"
    else
        ${ECHO} "${YELLOW}[-] Cannot list secrets engines (not authenticated or sealed)${NC}\n"
    fi

    # Vault agent config files
    VAULT_AGENT_CFG=$( DPRINT find /etc /opt /var /home /root -maxdepth 6 \
        -name 'vault-agent*.hcl' -o -name 'agent*.hcl' 2>/dev/null | xargs grep -l 'vault' 2>/dev/null | head -n 5 )
    if [ -n "$VAULT_AGENT_CFG" ]; then
        ${ECHO} "${BLUE}[+] Vault Agent Configs:${NC}"
        for agcfg in $VAULT_AGENT_CFG; do
            ${ECHO} "${YELLOW}$agcfg:${NC}"
            ${ECHO} "${ORAG}$( grep -vE '^\s*(#|$)' "$agcfg" 2>/dev/null )${NC}\n"
        done
    fi

else
    ${ECHO} "${YELLOW}[-] HashiCorp Vault not detected${NC}\n"
fi
